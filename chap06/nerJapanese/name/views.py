...
def index():
	...
	# # entityが「B-人名」もしくは「I-人名」のワードを抽出してnames_tokens配列に格納
	names_tokens = []
	for output in outputs:
		if output['entity'] == 'B-人名' or output['entity'] == 'I-人名':
			names_tokens.append(output['word'])
		else:
			continue

	# entityが「B-人名」の後に続く「I-人名」をentityに持つ単語を結合し、固有名詞として配列resultsに格納
	results = []
	for i in range(len(outputs)):
		if(outputs[i]['entity'] == 'B-人名'):
			name = outputs[i]['word']
			for j in range(i+1, len(outputs)):
				if outputs[j]['entity'] == 'I-人名':
					# #を取り除いて結合
